...
__global__ void kernel(float* r, float* g, float* b, float* r2, float* g2, float* b2, int N, int M, int x) {
    ...
    int des = (2*(tId%512)+1 - x);
    int res = ( des > N * M ) * 0 + ( des <= N * M ) * des;
        r2[res] = r[2*(tId%512)+1];
        g2[res] = g[2*(tId%512)+1];
        b2[res] = b[2*(tId%512)+1];
    }
}
int main(int argc, char const *argv[]) {
    ...
    // Solo para pruebas
    cudaMemcpy(R_gpu2, R, sizeof(float) * N * M, cudaMemcpyHostToDevice);
	cudaMemcpy(G_gpu2, G, sizeof(float) * N * M, cudaMemcpyHostToDevice);
	cudaMemcpy(B_gpu2, B, sizeof(float) * N * M, cudaMemcpyHostToDevice);
