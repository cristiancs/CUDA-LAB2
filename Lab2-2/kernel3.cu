...
__global__ void kernel(float* r, float* g, float* b, float* r2, float* g2, float* b2, int N, int M, int x) {
	...
	if(tId < N * M) {
    int c = tId % N;
    int des = ( ((c / x ) % 2 != 0)) * (tId - x) + ((c / x ) % 2 == 0) * (tId + x); // nueva posicion
...
int main(int argc, char const *argv[]) {
	...
	// Transponemos la entrada así a lo choro, onda, a lo vio, hrmno mira, lo hago altok

	for(int i=0, x = 0, y = 0; i< M * N; i++, x = i / N, y = i % N) 
		fscanf(in, "%f", &R[x * N + y]);
	...
	for(int i=0, x = 0, y = 0; i< M * N; i++, x = i / N, y = i % N) 
		fprintf(res, "%f%c", R[x * N + y], i == N * M - 1 ? '\n' : ' ');

	for(int i=0, x = 0, y = 0; i< M * N; i++, x = i / N, y = i % N) 
		fprintf(res, "%f%c", G[x * N + y], i == N * M - 1 ? '\n' : ' ');

	for(int i=0, x = 0, y = 0; i< M * N; i++, x = i / N, y = i % N) 
		fprintf(res, "%f%c", B[x * N + y], i == N * M - 1 ? '\n' : ' ');
